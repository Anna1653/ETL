...
Original file is located at
    https://colab.research.google.com/drive/1yJENl3MlBTqboWjRCI4DkeVuuQePJwXc
"""
...
# 3. Создайте три оператора для получения данных и загрузите файлы. Передайте дата фреймы в оператор трансформации;
    ...
    def hw8_get_bookings():
        bookings = pd.read_csv("\ДЗ\ETL-8\booking.csv")
        return bookings.to_json()

    @task
    def hw8_get_clients():
        clients = pd.read_csv("\ДЗ\ETL-8\client.csv")
        return clients.to_json()

    @task
    def hw8_get_hotels():
        hotels = pd.read_csv("\ДЗ\ETL-8\hotel.csv")
